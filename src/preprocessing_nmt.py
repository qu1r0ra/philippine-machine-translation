...
def split_and_export(
    df: pd.DataFrame, train_split: float = TRAIN_SPLIT, output_dir: Path = PROCESSED_DIR
) -> None:
    ...
    export_opennmt_files(train_df, "train", output_dir)
    export_opennmt_files(valid_df, "valid", output_dir)
